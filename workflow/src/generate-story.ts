...
import { isAudioEnabled, generateStory, getAudioStory, resultToStorySegments, storySegmentsToHTML } from './lib/story';
...
(async function run() {
  ...
  try {    
    const pastStories = pastStoryData.trim().split('---').map(story => story.trim());
